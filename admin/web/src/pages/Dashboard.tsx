...
// import { TodayStatistics, WeekStatistics, SystemInfo } from "@/types/api"; // 导入 SystemInfo
...
//         api.getSystemInfo({}) // 传入空对象以获取管理服务器信息
//       ]);
      
//       setStats(statsData);
//       setWeekStats(weekStatsData);
//       setSystemInfo(systemInfoData); // 设置系统信息

//     } catch (error) {
//       if (error instanceof Error) toast.error(error.message);
//       else toast.error("Failed to load dashboard data due to an unknown error");
//     } finally {
//       setIsLoading(false);
//     }
//   };
...
//     if (!systemInfo?.cpu?.cpus || systemInfo.cpu.cpus.length === 0) {
//       return 0;
//     }
//     const totalUsage = systemInfo.cpu.cpus.reduce((acc, usage) => acc + usage, 0);
//     return totalUsage / systemInfo.cpu.cpus.length;
//   }, [systemInfo]);
...
//                       <Label className="flex items-center gap-2 text-sm"><Cpu className="h-4 w-4"/> CPU Usage ({systemInfo?.cpu?.cores || 0} Cores)</Label>
//                       <Progress value={averageCpuUsage} />
//                       <p className="text-xs text-muted-foreground text-right">{averageCpuUsage.toFixed(1)}%</p>
//                     </div>
//                     {/* Memory */}
//                     <div className="space-y-2">
//                       <Label className="flex items-center gap-2 text-sm"><MemoryStick className="h-4 w-4"/> Memory (RAM)</Label>
...
//                      {/* Disk */}
//                     <div className="space-y-2">
//                       <Label className="flex items-center gap-2 text-sm"><HardDrive className="h-4 w-4"/> Disk Space</Label>
...
//                     <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/jobs")}><Calendar className="h-4 w-4 mr-2" /> View All Jobs</Button>
//                     <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/nodes")}><Server className="h-4 w-4 mr-2" /> Manage Nodes</Button>
//                     <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/logs")}><FileText className="h-4 w-4 mr-2" /> View Execution Logs</Button>
//                     <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/scripts")}><ScrollText className="h-4 w-4 mr-2" /> Manage Scripts</Button>
//                   </CardContent>
//                 </Card>
//             </div>
//         </div>

		...
		for _, ev := range wresp.Events {
			...
			case mvccpb.DELETE:
				...
				msg := &notify.Message{
					...
					Subject:   "节点失活警告",
					Body:      fmt.Sprintf("[Crony Warning] crony node[%s] in the cluster has filed, fail over success count: %d jobID are: %s, fail count: %d jobID are: %s", uuid, success.Count(), success.String(), fail.Count(), fail.String()),
...
func (n *NodeWatcherService) assignJob(nodeUUID string, job *models.Job) (err error) {
	...
	// 将任务信息写入etcd，路径为 /crony/job/<node_uuid>/<job_id>
...
func (n *NodeWatcherService) FailOver(nodeUUID string) (success Result, fail Result, err error) {
	...
	for _, job := range jobs {
		...
		// 3. 转移成功后，删除etcd中旧的键值对(/crony/job/<old_uuid>/<job_id>)
		_, err := etcdclient.Delete(fmt.Sprintf(etcdclient.KeyEtcdJob, oldUUID, job.ID))
		if err != nil {
...
func (n *NodeWatcherService) GetJobs(nodeUUID string) (jobs []models.Job, err error) {
	// 从etcd中获取指定前缀(/crony/job/profile/<node_uuid>/)下的所有键值对
...
func GetNodeSystemInfo(uuid string) (s *utils.Server, err error) {
	...
	defer func() {
		// 删除 /crony/system/switch/<uuid> 这个key
		_, err = etcdclient.Delete(fmt.Sprintf(etcdclient.KeyEtcdSystemSwitch, uuid))
	}()
	s = new(utils.Server)
	// 从etcd中读取节点写入的系统信息，key格式为/crony/system/get/<uuid>

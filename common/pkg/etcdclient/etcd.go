...
func Init(endpoints []string, dialTimeout, reqTimeout int64) (*Client, error) {
	client, err := clientv3.New(clientv3.Config{
		Endpoints:   endpoints,                                // etcd集群的地址列表
		DialTimeout: time.Duration(dialTimeout) * time.Second, // 建立连接超时时间
	})
	if err != nil {
	...
	_defaultEtcd = &Client{
		Client:     client,
		reqTimeout: time.Duration(reqTimeout) * time.Second, // 请求超时时间
	}
...
// 封装Put方法，简化了context的创建
func Put(key, val string, opts ...clientv3.OpOption) (*clientv3.PutResponse, error) {
	// 初始化检查
	if _defaultEtcd == nil {
		return nil, errors.ErrEtcdNotInit
	}
	ctx, cancel := NewEtcdTimeoutContext() // 创建一个带有超时和错误信息的context
	defer cancel()
...
// PutWithModRev 实现了基于版本号的CAS(Compare-And-Swap)更新
func PutWithModRev(key, val string, rev int64) (*clientv3.PutResponse, error) {
	...
	tresp, err := _defaultEtcd.Txn(ctx).
		...
		Then(clientv3.OpPut(key, val)). // Put
		Commit()
	cancel()
...
type etcdTimoutContext struct {
	context.Context
	etcdEndpoints []string
}

// 用于创建自定义的 etcdTimoutContext
func NewEtcdTimeoutContext() (context.Context, context.CancelFunc) {
	// 1. 获取etcd的请求超时时间
	ctx, cancel := context.WithTimeout(context.Background(), _defaultEtcd.reqTimeout)
	// 2. 创建etcdTimeoutContext实例
	etcdCtx := &etcdTimoutContext{}

	// 3. 将标准ctx赋值给etcdCtx
	etcdCtx.Context = ctx
	// 4. 获取etcd端点地址，并存入etcdCtx
	etcdCtx.etcdEndpoints = config.GetConfigModels().Etcd.Endpoints

	return etcdCtx, cancel
}

func (c *etcdTimoutContext) Err() error {
	err := c.Context.Err()
	// 检查是否为超时错误

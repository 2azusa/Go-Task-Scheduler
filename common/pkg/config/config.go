...
// var autoLoadLocalConfigs = []string{
// 	ExtensionJson,
// 	ExtensionYaml,
// 	ExtensionInI,
// }
...
func LoadConfig(env, serverName, configFileName string) (*models.Config, error) {
	var c models.Config
	var confPath string
	dir := fmt.Sprintf("%s/%s/%s", serverName, NameSpace, env) // 配置文件路径
	...
	v := viper.New()
	v.SetConfigFile(confPath) // 指定文件
	ext := utils.Ext(confPath)
	v.SetConfigType(ext)    // 指定文件类型
	err := v.ReadInConfig() // 读取配置文件到viper中
	if err != nil {
		panic(fmt.Errorf("FATAL ERROR CONFIG FILE: %s", err))
	}
